...
int Core::op_input(std::string fromMemName, std::string toMemName)
{
	...
	float *to = (float *)memhub.getMemory(toMemName).getPtr();
	for (int x = 0; x < 28; x++)
	{
		for (int y = 0; y < 28; y++)
		{
			to[x * 28 + y] = (float)from[x * 28 + y] * 1.0;
		}
	}
	return 10;
}
	...
	int fc)
{
	...
	return Rarray.calculate(input.getPtr(), output.getPtr(), fc);
}
	...
	std::string outMemName)
{
	Memory &input = memhub.getMemory(inMemName);
	Memory &output = memhub.getMemory(outMemName);
	return Rarray.sign(input.getPtr(), output.getPtr());
}